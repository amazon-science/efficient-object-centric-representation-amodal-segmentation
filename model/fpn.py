...
class FPN(nn.Module):
    def __init__(self, num_blocks, first_channel=3):
        super(FPN, self).__init__()
        #self.in_planes = 64

        self.conv1 = nn.Conv2d(
            first_channel, 64, kernel_size=7, stride=2, padding=3, bias=False)
    ...
    def load_pretrained(self, path, first_conv=True):
        ...
        for key, weights in pretrained.items():
            if first_conv:
                ...
                if key in state_dict and key!='conv1.weight':
                    state_dict[key].copy_(weights)
...
def FPN50(first_channel=3):
    fpn = FPN([3,4,6,3], first_channel)
    if first_channel!=3:
        first_conv=False
    else:
        first_conv=True
    fpn.load_pretrained(
        'https://download.pytorch.org/models/resnet50-19c8e357.pth',first_conv)
    return fpn
...
if __name__ == '__main__':
    x = torch.rand(1,3,375,1242)
    net = FPN50()
    y = net(x)
    for i in y:
        print(i.shape) #1*3*2*4
    #1*256*47*156
    #1*256*24*78
    #1*256*12*39
    #1*256*6*20
